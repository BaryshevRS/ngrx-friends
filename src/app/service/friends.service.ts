...
export class FriendsService {
  ...
  private findId(id: string, friendsList: Friend[]): Friend {
    return friendsList.find((friend: Friend) => id === friend.id) || new Friend();
  }
  ...
  private getRating(friendsList: Friend[]): Friend[] {
    const friendsRating: Record<string, any> = this.LocalSave.get(this.nameFriendsRating);
    if (friendsRating) {
  ...
  private getBookmark(friendsList: Friend[]): Friend[] {
    const friendsBookmark: Record<string, any> = this.LocalSave.get(this.nameFriendsBookmark);
    if (friendsBookmark) {
      return friendsList.map((friend, index) => {
        if (friend.id) {
